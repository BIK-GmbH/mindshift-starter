"""Mirrors `extension/lib/url.test.js`. Run from the backend directory:

    .venv/bin/python -m unittest tests.test_url_normalize -v

Every case here MUST also be in the JS test suite. If you add a case
here, add it there. The two implementations diverging is the worst
class of dedup bug because it manifests as silent duplicates.
"""
...
def _make_test(label: str, src: str, expected: str):
    def _t(self):
        self.assertEqual(canonicalize_url(src), expected)
...
for i, (label, src, expected) in enumerate(CASES):
    setattr(
        CanonicalizeUrlTest,
        f"test_{i:02d}_{label.replace(' ', '_').replace('-', '_').replace('/', '_')[:60]}",
        _make_test(label, src, expected),
    )
